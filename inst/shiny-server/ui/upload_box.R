upload_box <- function(width = 12, collapsible = T, collapsed = T,   # TODO: find a way to include all potential arguments
  ...
  box(
    ...
    sidebarPanel(
      ...
      hr(),
      actionButton("Upload.Add_to_repo", label = "Add uploaded data to repository"), #%>%
        # shinyInput_label_embed(
          custom_icon("exclamation-triangle") %>%
            bs_embed_tooltip(
              title = "Note that any user can see the repository, so don't upload any private data or data for
              which you don't have the distribution rights. We reserve the right to remove uploaded data at any
              time, and don't take responsibility for the integrity of data uploaded by any users."
            )
        # )
    )
    )
  ...
  box(
    ...
    sidebarPanel(
      ...
      HTML_P("Load the data from the available repositories. There are currently three available sources:
             <ul>
                <li>
                  Data generated with the PBO-suite, implemented in the 
                  <a href=https://github.com/IOHprofiler/IOHexperimenter>IOHexperimenter</a>
                </li>
                <li>
                  All data generated by the <a href=https://github.com/facebookresearch/nevergrad>nevergrad</a> 
                  benchmarking framework
                </li>
                <li>
                  The majority of the publicly available benchmark data on the single-objective 
                  <a href=https://github.com/numbbo/coco>BBOB</a> framework
                </li>
                <li>
                  A public data repository, containing algorithms uploaded by other users. This feature is still
                  in development, so we give no guarantees about the data in this repository.
                </li>
             </ul>"),
