...
repository_box <- function(width = 12, collapsible = F, collapsed = T, 
  ...
  box(
    ...
    sidebarPanel(
      ...
      selectInput('respository.type', label = "Select the dataset source",
                  choices = c("PBO", "NEVERGRAD"), selected = "PBO", width = '50%') %>%
        shinyInput_label_embed(
          custom_icon() %>%
            bs_embed_popover(
              title = "Datasets", 
              content = "These datasets contain data generated by the PBO suite of IOHexperimenter 
                         or by the NEVERGRAD continuous experimentation.", 
              placement = "left"
            )
        ),
