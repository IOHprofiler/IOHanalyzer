...
rt_ecdf_agg_targets_box <- function(width = 12, collapsible = T, collapsed = T) {
  box(
    ...
    sidebarPanel(
      ...
      checkboxInput('RT_ECDF_per_target', label = 'show ECDFs for each target', value = F),
      checkboxInput('RT_ECDF_AGGR_semilogx', label = 'scale x axis log10', value = F),
...
rt_ecdf_agg_fct_box <- function(width = 12, collapsible = T, collapsed = T) {
  box(
    ...
    sidebarPanel(
      ...
      fileInput(
        "CSV_Targets_upload", 
        label = HTML('<p align="left" style="font-size:120%;">Please choose 
                     a <i>csv file</i> containing the targets</p>'),
        multiple = FALSE, 
