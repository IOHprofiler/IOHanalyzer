...
tryCatch(
  {
  fmin <- as.numeric(REG$FCEECDF.AUC.Min[name])
  fmax <- as.numeric(REG$FCEECDF.AUC.Max[name])
  fstep <- as.numeric(REG$FCEECDF.AUC.Step[name])
...
p <- Plot.FV.ECDF_AUC(dsl_sub, fmin, fmax, fstep)
save_plotly(p, paste0('FV_ECDF_AUC_F', fid, '-', dim, 'D.pdf'), 'pdf', width = 1000, height = 500)
```
