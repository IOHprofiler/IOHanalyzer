...
# IMPORTANT! this function is only used to read some problematic COCO data set
...
# TODO: perhaps using naming scheme: read_dat_COCO, read_dat_IOH, read_dat_BIOBJ_COCO
# read_COCO_dat <- function(...) {
#   tryCatch(read_COCO_dat_IOH(...),
#            warning = function(e) read_BIOBJ_COCO_dat(...),
#            error = function(e) read_BIOBJ_COCO_dat(...),
#            finally = function(e) stop(paste0('Error in reading .dat files ', e)))
# }
...
read_COCO_dat <- function(fname, subsampling = FALSE) {
  ...
  c_read_dat(path.expand(fname), 5, '%')
}
