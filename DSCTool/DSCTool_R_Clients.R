...
eDSC_rank_service_parser<-function(desired_distribution,alpha,data_raw){
	DIM<-ncol(data_raw)-4
	algorithms_raw<-unique(as.character(data_raw$algorithm))
	problems_raw<-unique(as.character(data_raw$problem))
	...
	for(i in 1:length(algorithms_raw)){
		...
		problems[[j]]<-list(name=problems_raw[j],data=data)
	}
	l[[i]]<-list(algorithm=algorithms_raw[[i]],problems=problems)
	
	}

	method<-list(desired_distribution=desired_distribution,alpha=alpha)

	json_temp<-list(method=method,data=l)
	return(json_temp)
...
eDSC_client<-function(username,password,postfield){
	url<-paste(url_temp,"multivariate",sep="",collapse="")
	result_json<-POST(url,authenticate(username,password),add_headers(.headers = c('Content-Type'="application/json",'Accept' = "application/json")),body=postfield,encode="json")
	if(result_json$status_code==200){
		return(content(result_json)$result)
	}
...
performViz_json<-function(result,method){
	return(list(method=method,ranked_matrix=result$ranked_matrix))
}

performViz_client<-function(username,passwrod,postfield, dest_file){
	url<-paste(url_temp,"visualize",sep="",collapse="")	
	result<-POST(url,authenticate(username,password),add_headers(.headers = c('Content-Type'="application/json",'Accept' = "image/png")),body=postfield,encode="json")
	if(result$status_code==200){
		writeBin(result$content,dest_file)
	}
	else{
		return("Inccorect JSON!")
	}
}
...
#testing
DIM<-ncol(data_raw)-4
data_raw<-read.csv("/Users/Tome/Desktop/eDSC-10.csv",sep=";",header=TRUE)
algorithms_raw<-unique(as.character(data_raw$algorithm))
problems_raw<-unique(as.character(data_raw$problem))
...
result_json<-POST(url,authenticate(username,password),add_headers(.headers = c('Content-Type'="application/json",'Accept' = "application/json")),body=json_temp,encode="json")



png("/Users/Tome/Desktop/test_test.png")
url<-paste(url_temp,"visualize",sep="",collapse="")	
POST(url,authenticate(username,password),add_headers(.headers = c('Content-Type'="application/json",'Accept' = "image/png")),body=postfield,encode="json")
dev.off()


args<-list(username=username,password=password,body=postfield)

 headers<-c("application/json","image/png")
 names(headers)<-c('Content-Type','Accept')
 json<-"null"
 url<-"https://s.ijs.si:8443/dsc-1.5/service/visulize"
 
 
download.file(content(POST(url,authenticate(username,password),add_headers(.headers = c('Content-Type'="application/json",'Accept' = "image/png"))),body=postfield,encode="json"),"/Users/Tome/Desktop/myimagename.png",mode="wb")
